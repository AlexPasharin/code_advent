    ...
    FileReader::process_lines("./input/problem_5.txt", &mut |line| {
        ...
        match prev {
            ...
            Some(prev) => {
                ...
                let mut different_letter_pairs: HashSet<[char; 2]> = HashSet::new();
                ...
                for current_index in 1..chars.len() {
                    ...
                    if prev == curr {
                        ...
                        if let Some(prev_same_letter_twice_index) = same_letter_twice.get(&prev) {
                            contains_pair_twice = contains_pair_twice
                                || (*prev_same_letter_twice_index < current_index - 1);
                        } else {
                            same_letter_twice.insert(curr, current_index);
                        }
                    } else {
                        let consecutive_pair = [prev, curr];
