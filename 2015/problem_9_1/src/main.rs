...
fn main() {
    ...
    loop {
        ...
        if path.next_index < amount_of_destinations {
            ...
            // this approach saves some reculculations and also has the advantage to make it possible to stop generating a path half way through, if it is for example too long
            // we don't use that advantage here, but would be handy for example if we would only be interted in the shortest path
            ...
            for i in 0..amount_of_destinations {
        ...
        // case: this path cannot be continued, since all possibilies has already been explored but path is not empty
            ...
            // update next_index - find a next available index after "head"m which was just removed
            path.next_index = amount_of_destinations;

            for i in head + 1..amount_of_destinations {
                if !path.used_destinations.contains(&i) {
                    path.next_index = i;
